...
from gestor_legajos.utils.persistencia import obtener_directorio_out, obtener_ruta_templates
from gestor_legajos.entidades.empresa import Empleado, Domicilio, Empresa
...
def formulario_galeno(empleado: Empleado, empresa: Empresa):
    pdf_path = obtener_ruta_templates("pdf_galeno.pdf")
...
def formulario_mapfre(empleado: Empleado, empresa: Empresa):
    pdf_path = obtener_ruta_templates("pdf_mapfre.pdf")
    pdf = pymupdf.open(pdf_path)
...
def formulario_andina(empleado: Empleado, empresa: Empresa):
    pdf_path = obtener_ruta_templates("pdf_andina.pdf")
    pdf = pymupdf.open(pdf_path)
    pagina = pdf[0]
    tamaño_fuente = 10
